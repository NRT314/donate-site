console.log('script.js ЗАПУЩЕН!'); // Тестовая строка для проверки запуска скрипта
...
function updateWalletUI(address, isWrongNetwork = false) {
    ...
    if (address) {
        ...
        if (connectButtonContainer) connectButtonContainer.style.display = 'none';
        ELEMENTS.disconnectBtn.style.display = 'block';
    } else {
        ELEMENTS.walletAddressEl.innerText = '';
        if (connectButtonContainer) connectButtonContainer.style.display = 'block';
